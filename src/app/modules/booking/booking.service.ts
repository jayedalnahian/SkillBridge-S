...
import { BOOKING_DEFAULT_INCLUDES, BOOKING_FILTERABLE_FIELDS, BOOKING_INCLUDE_CONFIG, BOOKING_SEARCHABLE_FIELDS } from "./booking.constant";
...
const createBooking = async (
    ...
    if (isNaN(start.getTime()) || isNaN(end.getTime())) {
        throw new AppError(status.BAD_REQUEST, "Invalid startDateTime or endDateTime");
    }
    ...
    if (start < new Date()) {
        throw new AppError(status.BAD_REQUEST, "Booking cannot be scheduled in the past");
    }
    ...
    if (tutor.status !== "ACTIVE") {
        throw new AppError(status.BAD_REQUEST, "Tutor is not currently accepting bookings");
    }
...
const getBookings = async (user: IRequestUser, queryParams: IBookingQueryParams) => {
